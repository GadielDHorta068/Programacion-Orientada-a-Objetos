...
class TestDeliveryParcial {
	...
	static void setUpBeforeClass() throws Exception {
		...
		Delivery.setComisionVentaRealizada(300);
	}
	...
	@Test
	void testCobroCliente2() {
		Pedido pedido = pedidosAhora.agregarPedido(bomke, ana, LocalDateTime.now(), "");
		pedido.agregarItem(bk2, 1);
		pedido.agregarItem(bk3, 2);
		assertEquals(1350 * 2 + 760 * 1 + 200, pedido.calcularPago(), 0.01);
	}

	@Test
	void testPagoRepartidor() {
		...
		assertEquals(200 * 2, pablo.calcularPago(), 0.01);
		assertEquals(0, maria.calcularPago(), 0.01);
